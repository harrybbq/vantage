...
exports.handler = async (event) => {
  ...
  const tokRes = await sb('oura_tokens?select=user_id&order=updated_at.asc', {}, env);
  if (!tokRes.ok) {
  ...
  console.info(`oura-cron: ${synced} synced, ${skipped} skipped, ${failed} failed, ${deferred} deferred to the next run, of ${rows.length}`);
  return { statusCode: 200 };
};
