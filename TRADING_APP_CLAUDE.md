...
> **Copy this into the new repository as `CLAUDE.md`.** It is written for
> a fresh Claude Code session that has no prior context. It carries the
> decisions already made so they aren't relitigated, and the reasoning
> behind them so they can be overturned deliberately rather than by
> accident.
